...
%% Parameterize A_rest, t1_rest, t2, Km_rest

% %% Parameterize aA, aKm, aT1, t_fat
...
sset = ceil(linspace(find(pwindow,1,'first'),find(pwindow,1,'last'),200));
plot(time(sset)/1000,sum(Fnew(sset,:),2), '.', 'color', bloo, 'markersize', 10)
...
ylabel('Force (N)')
xlabel('Time (s)')
legend(["Summed MU Forces", "Ding Model"])
...
subplot(2,2,[3 4])
hold on
plot(time(imaxD03)/1000, fmaxD03,'-', 'color', reed,'linewidth',1.2)
plot(time(indnewmax(:,1))/1000, sum(Fnewmax,2),'.', 'color', bloo, 'markersize', 10)
text(0.05,1,char('C'),'Units', 'Normalized', 'VerticalAlignment', 'Top', 'FontSize', 20)
title('Maximum Force -- Original vs Parameterized')
ylabel('Force (N)')
xlabel('Time (s)')
axis([-inf inf 0 max(F_Ding03)*1.5])
legend(["Ding Model", "Summed MU Forces"])
% subplot(2,2,4)
% hold on
% for mu = 1:nMu
%     pmu = nMu - mu +1
...
pwindow = time < window_len * 2;
for mu = 1:nMu
    plot(time(pwindow)/1000, Fnew(pwindow,mu), '-','color', scale_gradient(nMu - mu + 1,:))
    hold on
end
plot(time(pwindow)/1000,sum(Fnew(pwindow,:),2), '--', 'color', reed)
% plot(time(pwindow)/1000,F_Ding03(pwindow), '--', 'color', reed)
legend(['MU' + string([1:nMu]), "Sum"],'Location', 'eastoutside')
axis([-inf inf 0 max(F_Ding03)*1.1])
text(0.015,0.95,char('A'),'Units', 'Normalized', 'VerticalAlignment', 'Top', 'FontSize', 20)
title('Individual Motor Unit and Summed Forces')
ylabel('Force (N)')
xlabel('Time (s)')

subplot(2,1,2)
for mu = 1:nMu
    plot(time(indnewmax(:,1))/1000, force_prop(:,mu)*100, '-','color', scale_gradient(nMu - mu + 1,:))
    hold on
end
plot(time(indnewmax(:,1))/1000, percent_decline*100, '--','color', bloo)
legend(['MU' + string([1:nMu]), "Overall"],'Location', 'eastoutside')
axis([-inf inf 0 100])
text(0.015,.25,char('B'),'Units', 'Normalized', 'VerticalAlignment', 'Top', 'FontSize', 20)
title('Motor Unit Fatigue Curves')

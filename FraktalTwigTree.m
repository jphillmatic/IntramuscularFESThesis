function [neuropoints] = FraktalTwigTree(n,prevNode,level,node) 
%% Input Arguments:
...
if level == 1
    n_branches = round(3.5 + randn(1)*0.58);
end
...
if n_branches > 1
    ...
    for i = 2:n_branches
        ...
        phi(i) = phi0 + pi/8*randn(1)/n_branches;
    end
end

if level == 1
    chi = pi / 6 + pi/12*randn(n_branches,1);

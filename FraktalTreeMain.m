...
while (mu < dmu*nMu)
    neuropoints = FraktalTwigTree872018(n);
    neuropoints = removeOutliers(neuropoints);
    nmj_nodes = find_nmj(neuropoints);
    mu = length(nmj_nodes);
    
    nmj_pts = subset_tree(neuropoints, nmj_nodes);
    xnt = ~kstest(nmj_nodes(:,1),'alpha',0.1);
    ynt = ~kstest(nmj_nodes(:,2),'alpha',0.1);
...
rescale_mod = 15/ht;
neuropoints(:,1:3) = neuropoints(:,1:3)*rescale_mod;
n_nodes = max(neuropoints(:,6));

[x0, y0, r, ht] = find_cyl(neuropoints);

% n_outside = removeOutliers(neuropoints);
% neuropoints = subset_tree(neuropoints, neuropoints(~ismember(neuropoints(:,6),n_outside),6));
nmj_nodes = find_nmj(neuropoints);
...
n_arborLengths = ceil(arborLengths * 0.10 * tree_length);
...
%     plot_tree(npnew, [np_activated{dx}], scale_discrete(2,:));
% %     
% %     %     pause(3)
...
scatter3(rr(:),zeros(length(rr(:)),1), elec_z(:), flip(distnc(:),1)*400+1,'r')
hold off
view([0 0])
%%
toc

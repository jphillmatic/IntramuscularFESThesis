...
%%
cycle_position_plot(npnew,mu_tree,active_mu_bool,e_pts, axis_vals, percent_force_recruitment', 'PositionSubplot')
%%
i_stims = linspace(0,15, 30)*1e-3;
active_mu_bool = cycle_current_plot(npnew,mu_tree,i_stims,[]);
percent_force_recruitment = sum(active_mu_bool.*P,2)/sum(P);
figure(2)
hold on
plot(i_stims*1e3,percent_force_recruitment,'--','color',bloo)
plot(i_stims*1e3,percent_force_recruitment,'.','color',reed)
xlabel('Stimulation Amplitude (mA)')
ylabel('Percent of Total Muscle Force Recruitment')
title('Current vs Percent Force Recruitment')
%%
% distnc(1) = 1;
figure(10)
clf
rmp = RHO./r;
zmp = flip(Z./ht);
h = heatmap(rmp,zmp, distnc,'CellLabelColor','k', 'CellLabelFormat','%0.2f');
h.Title = 'Proportion of Total Force Recruitment';
h.XLabel = 'Normalized Radial Position';
h.YLabel = 'Normalized Proximal-Distal Position';
set(gcf, 'Position', [0, 0, 600*1.2, 600])
export_fig 'C:\Users\u153094\Documents\UPF\GradSchool\Thesis Project\Paper Files\Figures\RecruitmentHeatmap.png' -transparent -m3
